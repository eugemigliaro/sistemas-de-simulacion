...
Esta vista resume cómo se conectan las responsabilidades del proyecto. El barrido de experimentos ejecuta el motor en C++ una vez por combinación de parámetros, y el motor produce los datos crudos de cada corrida. Esos datos alimentan el cálculo estadístico de los observables y la generación de las visualizaciones. El diagrama separa así el motor de simulación de sus consumidores: el motor resuelve la evolución del sistema, mientras que el análisis y el graficado leen sus resultados. En la diapositiva siguiente vemos la organización interna del motor con más detalle.
