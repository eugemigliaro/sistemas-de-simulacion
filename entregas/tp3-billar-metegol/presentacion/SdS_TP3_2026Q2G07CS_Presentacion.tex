...
\begin{frame}{Dinamica del sistema}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      ...
      \videolink{URL YouTube/Vimeo pendiente de publicacion}
    \end{column}
    \begin{column}{0.47\textwidth}
      \centering
...
\begin{frame}{Tiempo de ejecucion en funcion de $N$}
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.28\textwidth}
      \parametros{Mesa vacia\\$t_f=30\ \mathrm{s}$\\$n_r=10$ por $N$\\Motor: cola de prioridad}
      \vspace{0.8em}
      \begin{block}{Lectura}
        En el rango medido, el ajuste para $N\geq50$ da $t\propto N^{3{,}30}$; a $N=400$, $t=11{,}27\pm0{,}04$ s.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Evolucion de la fraccion usada}
  \begin{columns}[c,onlytextwidth]
    ...
    \end{column}
    \begin{column}{0.28\textwidth}
      \parametros{$N=100$\\Mesa vacia: semilla 2006\\Elegida: semilla 2010}
      \vspace{0.8em}
      \begin{block}{Escalar}
...
\begin{frame}{Exploracion de configuraciones}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.68\textwidth}
      \includegraphics[width=\textwidth,height=5.25cm,keepaspectratio]{figuras/t90-vs-configuracion.pdf}
    \end{column}
    \begin{column}{0.28\textwidth}
      \parametros{$N=100$\\$n_r=15$ por punto\\Variables: $x_k$ y $R_k$\\$t_{\max}=100\ \mathrm{s}$}
      \vspace{0.6em}
      \begin{block}{Elegida}
        $K=1$\\$(x_k,y_k,R_k)=(0{,}60;0{,}34;0{,}34)\ \mathrm{m}$\\
        $\langle t_{90}\rangle=15{,}54\pm1{,}55\ \mathrm{s}$
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Coeficiente de difusion}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \includegraphics[width=\textwidth,height=4.2cm,keepaspectratio]{figuras/DCM-ajuste.pdf}
    \end{column}
    \begin{column}{0.48\textwidth}
      \includegraphics[width=\textwidth,height=4.2cm,keepaspectratio]{figuras/error-ajuste.pdf}
    \end{column}
  \end{columns}
  \vspace{0.35em}
  \centering\parametros{Configuracion elegida; 5 semillas; ventana $[0{,}3;1{,}5]$ s; $\langle D\rangle=(0{,}00752\pm0{,}00036)\ \mathrm{m^2\,s^{-1}}$.}
\end{frame}
...
\begin{frame}{Difusion y tiempo al 90\,\%}
  \begin{columns}[c,onlytextwidth]
    \begin{column}{0.67\textwidth}
      \includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{figuras/D-vs-t90.pdf}
    \end{column}
    \begin{column}{0.29\textwidth}
      \parametros{$N=100$\\Mismas configuraciones\\$n_r=15$ para $t_{90}$\\$n_r=5$ para $D$\\Barras: error estandar}
      \vspace{0.8em}
      \begin{block}{Relacion observada}
        No aparece una correlacion global relevante ($r=0{,}13$): la geometria importa mas que $D$ por si solo.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusiones}
  \begin{itemize}
    \item El costo crecio como $N^{3{,}30}$ en el rango $50\leq N\leq400$; la cola no evita el fuerte aumento de eventos a alta densidad.
    \item El obstaculo central de radio maximo redujo $\langle t_{90}\rangle$ un 31,5 por ciento y tambien redujo su dispersion frente a la mesa vacia.
    \item $D$ no predijo por si solo el tiempo al 90 por ciento: en el barrido completo la correlacion fue debil ($r=0{,}13$).
  \end{itemize}
\end{frame}
